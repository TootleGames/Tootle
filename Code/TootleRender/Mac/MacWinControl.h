...
namespace Win32
{
	...
	namespace GWinControlFlags
	{
		//	style flags
		const u32	Popup						= 0;
		const u32	OverlappedWindow			= 0;
		const u32	Visible						= 0;
		const u32	Child						= 0;
		const u32	ClipSiblings				= 0;
		const u32	VertScroll					= 0;
		const u32	HorzScroll					= 0;
		const u32	TreeView_DrawLines			= 0;
		const u32	TreeView_DrawButtons		= 0;
		const u32	TreeView_DrawLinesAtRoot	= 0;
		const u32	TreeView_EnableLabelEdit	= 0;
		const u32	TreeView_DisableDragDrop	= 0;
		//	style flag groups
		...
		//	ex style flags
		const u32	ClientEdge					= 0;
		const u32	AlwaysOnTop					= 0;
		const u32	AllowDragDrop				= 0;
		const u32	ToolWindowBorder			= 0;
		//	Ex flags groups
...
public:
	...
	virtual u16				ClassStyle()							{	return 0;	};
	virtual Bool			ClassExists();

	//	platform system notify messages
...
//------------------------------------------------
class Win32::TWinControlFactory : public TObjectFactory<Win32::GWinControl>
{
protected:
	virtual Win32::GWinControl*		CreateObject(TRefRef InstanceRef,TRefRef TypeRef);
};
