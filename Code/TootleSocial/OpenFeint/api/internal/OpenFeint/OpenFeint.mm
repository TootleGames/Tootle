					 ...
					 andDelegates:(OFDelegatesContainer*)delegatesContainer
{
    ...
    if(OFSettings::Instance()->getDebugOverrideKey()) productKey = OFSettings::Instance()->getDebugOverrideKey();
    if(OFSettings::Instance()->getDebugOverrideSecret()) productSecret = OFSettings::Instance()->getDebugOverrideSecret();
...
- (void) dealloc
{
    ...
    OFSafeRelease(_OS32bugDummyController);
...
+ (void)presentUserFeintApprovalModal:(OFDelegate&)approvedDelegate deniedDelegate:(OFDelegate&)deniedDelegate
{
	...
	else
	{
        ...
        OFUserFeintApprovalController* nativeController = nil;
        
        // If we are online, use a webview controller
        ...
        // If we are offline use the old native controller
        else
        {
            nativeController = (OFUserFeintApprovalController*)OFControllerLoader::load(@"UserFeintApproval");
            [nativeController setApprovedDelegate:approvedDelegate andDeniedDelegate:deniedDelegate];
            modal = nativeController;
        }
