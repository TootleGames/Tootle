...
+(void)downloadS3Blob:(NSString*)url onSuccess:(const OFDelegate&)onSuccess onFailure:(const OFDelegate&)onFailure
{
	[OFCloudStorageService downloadS3Blob:url passThroughUserData:nil onSuccess:onSuccess onFailure:onFailure];
}

+(void)downloadS3Blob:(NSString*)url passThroughUserData:(NSObject*)userData onSuccess:(const OFDelegate&)onSuccess onFailure:(const OFDelegate&)onFailure
{
	...
	httpService->startRequest(url, HttpMethodGet, nil, nil, nil, userData, nil, new OFBlobDownloadObserver(chainedSuccess, onFailure, true));
}
