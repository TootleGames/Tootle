...
public:
	...
	virtual TPtr<TFile>		CreateFile(const TString& Filename,TRefRef FileTypeRef);	//	create a new empty file into file system if possible - if the filesys is read-only we cannot add external files and this fails
	virtual SyncBool		DeleteFile(TRefRef FileRef);	//	delete file from file sys
};
