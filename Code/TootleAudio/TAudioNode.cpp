...
Bool TAudioNode::CreateSource()
{
	if(Platform::CreateSource(GetNodeRef()))
	{
		...
		{
			// TEMP
			// Create the buffer and attach to the source
			// If buffer creation fails then release the node.
			if(CreateBuffer())
			{
				// Bind the source to the buffer
				return Platform::AttachSourceToBuffer(GetNodeRef(), m_AudioAssetRef, GetIsStreaming());
...
Bool TAudioNode::CreateBuffer()
{
	TPtr<TLAsset::TAudio> pAudioAsset = TLAsset::GetAssetPtr(TTypedRef(m_AudioAssetRef, "audio"));

	if(!pAudioAsset)
		return FALSE;

	return Platform::CreateBuffer(*pAudioAsset.GetObjectPointer());
}
