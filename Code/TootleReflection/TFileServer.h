...
#include <TootleNetwork/TSocketServer.h>
...
public:
	...
	virtual void		OnPeerConnected(TPtr<TLNetwork::TSocketServer>& pSocket,TRefRef Peer);	//	our client has connected on the data connection, send out the queued packet (if there is one). returns false if there is a problem and we need to reject the connection (eg. peer is already connected)
...
private:
	...
	TPtr<TLNetwork::TSocketServer>	m_pSocket;		//	socket to send data on
};
...
public:
	...
	void		Update(TLNetwork::TSocketServer& Server);				//	update routine, required to catch the finishing of a data transfer

	TRefRef		GetPeerRef() const					{	return m_PeerRef;	}

	bool		SendPacket(u32 FtpCommand,const TString& String,TLNetwork::TSocketServer& Server);
	...
	bool		ExecuteCommands(TLNetwork::TSocketServer& Server);		//	execute any/all the commands we've put onto the client. return false if we want to disconnect the client

protected:
	virtual TRefRef		GetSubscriberRef() const				{	static TRef Ref("FtpTask");	return Ref;	}
	virtual void		ProcessMessage(TLMessaging::TMessage& Message);

	bool		ExecuteCommand(const TFtpCommand& Command,TLNetwork::TSocketServer& Server);		//	execute a command, return false if we want to close the connection
	void		OnDataTaskFinished(bool Success,TLNetwork::TSocketServer& CommandServer);	//	current data task has finished
...
protected:
	...
	TPtr<TLNetwork::TSocketServer>	m_pDataServer;		//	data server socket for this client - gr: this could be stored on the task, (for multiple transfers at once) but I've stored it here to re-use it
...
protected:
	TPtr<TLNetwork::TSocketServer>	m_pServer;					//	command server socket
